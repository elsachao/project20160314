...
#13
x12<-factor(c("yes", "yes", "no", "yes", "no")) #no是1,yes是2
x12
x12a<-factor(c("yes", "yes", "no", "yes", "no"), levels =c("yes","no"))
x12a #指定yes在前, 用於有排序的東西

#14
x13 <- data.frame(foo = 1:4, bar = c(T, T, F, F)) #2個col,4個row 
x13 #column names: foo, bar 
nrow(x13) #how many row?
ncol(x13)
names(x13)<-c("fooNew","barNew")#rename column name
x13
...
names(NBA1415)#show the column name
head(NBA1415) #see the first 6 ob.
nrow(NBA1415) #how many row?
ncol(NBA1415)
tail(NBA1415)#see the last 6 ob.

#17
x16<-as.Date("1970-01-01") #Stored as number of days since 1970/01/01
x16
unclass(x16) ##Stored as number of days since 1970/01/01
unclass(as.Date("1971-01-01"))
weekdays(x16)
months(x16)
seq(Sys.Date(), by="1 months", length.out=12)#現在時間每隔1個月產生12個
...
paste(c("Hello", "World"), sep='',collapse = '') #handle 向量
#字串切割
substr("Hello World", start=2,stop=5)
#字串取代
gsub("o","0","Hello World") #str can be 向量
